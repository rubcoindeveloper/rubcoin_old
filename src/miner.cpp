...
// Copyright (c) 2018-2019 The Rubcoin developers
...
CBlock* CreateNewBlock(CWallet* pwallet, CTransaction *txCoinStake)
{
    ...
    int64_t nFees = 0;
    {
        ...
        if (!fProofOfStake)
        {
            pblock->vtx[0].vout[0].nValue = GetProofOfWorkReward(pblock->nBits, pindexPrev->nHeight+1, nFees);
...
void ThreadStakeMiner(void* parg)
{
    ...
    uint32_t nHeight = pindexPrev->nHeight;
    ...
    try
    {
        ...
        do
        {
            ...
            if (ScanMap(inputsMap, nBits, LuckyInput, solution))
            {
                ...
                if (!pwallet->CreateCoinStake(LuckyInput.first, LuckyInput.second, solution.second, nBits, nHeight, txCoinStake, key))
                {

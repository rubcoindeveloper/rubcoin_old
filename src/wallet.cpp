    ...
    while (fRepeat)
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CWalletTx)& item, mapWallet)
        {
            ...
            if (txdb.ReadTxIndex(wtx.GetHash(), txindex))
            {
                ...
                if (fUpdated)
                {
                    printf("ReacceptWalletTransactions found spent coin %s RBC %s\n", FormatMoney(wtx.GetCredit(MINE_ALL)).c_str(), wtx.GetHash().ToString().c_str());
                    wtx.MarkDirty();
                    wtx.WriteToDisk();
                }
            }
...
bool CWallet::CreateCoinStake(uint256 &hashTx, uint32_t nOut, uint32_t nGenerationTime, uint32_t nBits, uint32_t nHeight, CTransaction &txNew, CKey& key)
{
    CWalletTx wtx;
    ...
    int64_t nCombineThreshold = GetProofOfWorkReward(GetLastBlockIndex(pindexBest, false)->nBits, GetLastBlockIndex(pindexBest, false)->nHeight) / 3;
    ...
    nCredit += GetProofOfStakeReward(nCoinAge, nBits, nHeight, nGenerationTime);
...
{
    {
        ...
        if (nSize > 0)
            ...
            nKeys = max<uint64_t>(GetArg("-keypool", 500), 0);
...
{
    {
        ...
        if (nSize > 0)
            ...
            nTargetSize = max<uint64_t>(GetArg("-keypool", 500), 0);

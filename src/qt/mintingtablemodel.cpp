...
QString MintingTableModel::formatTxPoSReward(KernelRecord *wtx) const
{
    ...
    int nHeight = GetLastBlockIndex(pindexBest, true)->nHeight;
    posReward += QString(QObject::tr("%1")).arg(BitcoinUnits::formatWithUnit(walletModel->getOptionsModel()->getDisplayUnit(), wtx->getPoSReward(nBits, nHeight, mintingInterval))); 
    return posReward;
}

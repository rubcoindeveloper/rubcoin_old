...
int64_t GetProofOfStakeReward(int64_t nCoinAge, unsigned int nBits, unsigned int nHeight, int64_t nTime, bool bCoinYearOnly)
{
    int64_t nSubsidy = 0;

    if (fDebug && GetBoolArg("-printcreation") && !bCoinYearOnly)
        printf("GetProofOfStakeReward(): create=%s nCoinAge=%" PRId64 " nBits=%d nHeight=%d\n", FormatMoney(nSubsidy).c_str(), nCoinAge, nBits, nHeight);

    return nSubsidy;
}

static const int64_t nTargetTimespan = 10 * 60;  // 10 blocks
...
{
    if(fTestNet)
        return 20; // 20 seconds on testNet

    return nStakeTargetSpacing; // 1 min on mainNet
}
    ...
    if (!IsCoinBase())
    {
        ...
        if (IsCoinStake())
        {
            if (nTime >  Checkpoints::GetLastCheckpointTime())
            {
                ...
                int64_t nCalculatedReward = GetProofOfStakeReward(nCoinAge, pindexBlock->nBits, pindexBlock->nHeight, nTime) - GetMinFee(1, false, GMF_BLOCK, nTxSize) + CENT;
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in Rubcoin
    ...
    if (IsProofOfWork())
    {
        int64_t nBlockReward = GetProofOfWorkReward(nBits, pindex->nHeight, nFees);
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
    ...
    if (fProofOfStake)
    {
        ...
        // Rubcoin: check proof-of-stake block signature
...
bool CBlock::AcceptBlock()
{
    ...
    // Reject non-PoS blocks
    if (IsProofOfWork() && nHeight >= LAST_POW_HEIGHT)
        return DoS(100, error("AcceptBlock() : proof-of-work is not accepted by the network at this point"));
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Rubcoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    if (fTestNet)
    {
        ...
        nModifierInterval = 5 * 60; // test modifier interval is 5 minutes
        nCoinbaseMaturity = 5; // test maturity is 5 blocks
        nStakeTargetSpacing = 20; // test block spacing is 20 sec
    }
        ...
        BOOST_FOREACH(const CInv& inv, vInv)
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    {
                        // ppcoin: send latest block to allow the download node
                        // to accept as orphan (proof-of-stake block might be 
                        // rejected by stake connection check)
                        vector<CInv> vInv;

...
Value getsubsidy(const Array& params, bool fHelp)
{
    ...
    return (uint64_t)GetProofOfWorkReward(nBits, nHeight);
}
...
Value getmininginfo(const Array& params, bool fHelp)
{
    ...
    obj.push_back(Pair("blockvalue",    (uint64_t)GetProofOfWorkReward(GetLastBlockIndex(pindexBest, false)->nBits, GetLastBlockIndex(pindexBest, false)->nHeight)));
    ...
    obj.push_back(Pair("stakeinterest", GetProofOfStakeReward(0, GetLastBlockIndex(pindexBest, true)->nBits, GetLastBlockIndex(pindexBest, true)->nHeight, GetLastBlockIndex(pindexBest, true)->nTime, true)));
...
Value getworkex(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "Rubcoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "Rubcoin is downloading blocks...");

    if (pindexBest->nHeight >= LAST_POW_HEIGHT)
        throw JSONRPCError(RPC_MISC_ERROR, "proof-of-work is not accepted by the network at this point");
